...
#
# Copyright (c) 2008 Timekpr Authors.
...
def getconfsection(conffile):
    ...
    #TODO:ELIMINAMI-Why compiling the pattern and do not saving it?? It's unuseful? Simpler:
    #TODO:ELIMINAMI-check = re.findall('## TIMEKPR START|## TIMEKPR END',s)
    ...
    #TODO:ELIMINAMI-m = re.findall('## TIMEKPR START\n(.*)## TIMEKPR END', s, re.S) #falg:re.S il . match all anche newline
    #TODO:ELIMINAMI-ritorna tutto cio all'interno di () che sia '.' un carattere qualsiasi più '*' tutto quello che viene dopo
    m = re.compile('## TIMEKPR START\n(.*)## TIMEKPR END', re.S).findall(s)
    #Di 0 Ma è composto da un solo elemento
    return m[0]
...
def isuserlocked(u):
    ...
    """
    #TODO:ELIMINAMI-Inutile assegnare a 'i' funziona lo stesso
    try:
...
def converttconf(tfrom, tto, mode=0):
    ...
    """
    if mode == 0:
	#TODO:ELIMINAMI Si può togliere list? ritorna una lista?
...
def parseutlist(utlist):
    ...
    for u,t in utlist:
        ...
        #TODO:ELIMINAMI-Se lo trova torna un mAtch Object
        if checkAl:
